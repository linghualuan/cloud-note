...
class QiniuManager {
    ...
    generateDownLoadLink(key) {
        const dominPromise = this.publicBucketDomain ? Promise.resolve([this.publicBucketDomain]) : this.getBucketDomain()
        return dominPromise.then(data => {
    ...
    downloadFile(key, downloadPath) {
        return this.generateDownLoadLink(key).then(link => {
            ...
            return new Promise((resolve, reject) => {
                writer.on('finish', () => resolve('成功'))
                writer.on('error', () => reject('失败'))
            })
        }).catch(err => {
